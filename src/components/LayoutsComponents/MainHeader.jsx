...
const MainHeader = ({ setCollapsed, collapsed }) => {
    ...
    const handleProfileClick = () => {
        if (user?.role) {
            switch (user.role) {
                case 'super_admin':
                    navigate('/super_admin/profile');
                    break;
                case 'support-agent':
                    navigate('/support-agent/profile');
                    break;
                case 'location_employee':
                    navigate('/location_employee/profile');
                    break;
                case 'third_party':
                    navigate('/third_party/profile');
                    break;
                case 'organization':
                ...
                <div className=' flex justify-between pr-4 bg-white text-black'>
                    ...
                    <div className='flex items-center  space-x-2 '>
                        ...
                        <div onClick={() => navigate('profile')} className='flex items-center space-x-2 cursor-pointer'>
                            <div onClick={() => handleProfileClick()} className='h-[42px] w-[42px] rounded-full pt-1 cursor-pointer'>
                                <img className='h-[40px] w-[40px] rounded-full' src={adminProfile?.image} alt="" />
                            </div>
                            <strong className='text-[20px] font-semibold text-primary'>{adminProfile?.name}</strong>
                        </div>
                    </div>
                </div>
            </Header>
