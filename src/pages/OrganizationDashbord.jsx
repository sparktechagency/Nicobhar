...
export default function OrganizationDashbord() {
  ...
  // Transform out of order assets data for the pie chart
  const assetData = organizationData?.["out_of_order_assets    "]?.asset_data?.map(item => ({
    name: item.asset,
    value: item.quantity,
    color: getRandomColor()
  })) || []

  // Transform warranty data for the pie chart
  const warrantyData = organizationData?.warranty_details?.asset_data?.map(item => ({
    name: item.product,
    value: 1, // Since we're showing each warranty item as equal segments
    expiry: item.expiry_text,
    color: getRandomColor()
  })) || []

  // Transform ticket status data for the pie chart
  const ticketData = organizationData?.ticket_status?.ticket_data?.map(item => ({
    name: item.tickets,
    value: item.quantity,
    color: getRandomColor()
  })) || []
  ...
  console.log(organizationData?.["out_of_order_assets    "]?.asset_data)


  // Helper function to generate random colors for the pie chart
  function getRandomColor() {
    const colors = ['#4364E8', '#FFA500', '#32CD32', '#FF69B4', '#D3D3D3', '#800080', '#008080', '#FF0000', '#00FF00', '#0000FF']
    ...
    <div className="p-6 space-y-6">
        ...
        <Card>
          <div className="py-6">
            <div className="flex flex-col space-y-2 text-center">
              ...
              {/* dynamic data by server-site */}
              <div className="text-2xl font-bold">{organizationData?.total_ticket}</div>
            </div>
          </div>
        </Card>
        ...
        <Card>
          <div className="py-6">
            <div className="flex flex-col space-y-2 text-center">
              ...
              {/* dynamic data by server-site */}
              <div className="text-2xl font-bold">{organizationData?.total_job_card}</div>
            </div>
          </div>
        </Card>
        ...
        <Card>
          <div className="py-6">
            <div className="flex flex-col space-y-2 text-center">
              ...
              {/* dynamic data by server-site */}
              <div className="text-2xl font-bold">{organizationData?.total_inspection_sheet}</div>
            </div>
          </div>
        </Card>
      </div>
      ...
      <Modal
        title={
          <div className="modal-header text-center w-full absolute top-0 left-0 rounded-t-lg flex items-center justify-center">
            <span className="header-title text-[#F0F0F0] flex  text-[20px] font-normal">Organization name</span>
          </div>
        }
        open={isModalOpen}
      ...
      <div className="">
        <MetricsCard
          ...
          total={organizationData?.["out_of_order_assets    "].total || 0}
        />
        <MetricsCard
          ...
          total={organizationData?.warranty_details?.total || 0}
        />
        <MetricsCard
          title="Ticket Status"
          data={ticketData}
          color="bg-orange-500"
          columns={["Tickets", "Quantity"]}
          total={organizationData?.ticket_status?.total || 0}
        />
      </div>
    </div>
  )
...
function MetricsCard({ title, data, color, columns, total }) {
  return (
    <div className="gap-4 rounded-lg bg-white shadow-sm">
      <div className={`${color} text-white p-6 h-full rounded-lg`}>
        <h1 className="text-[16px] font-semibold p-4">{title}</h1>
        <p className="text-xl font-bold px-4 pb-4">Total: {total}</p>
      </div>
      <div className="p-6 mb-8">
        <div className="flex justify-center items-center gap-10 h-[400px] ">
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-center mt-10 gap-1  font-medium">
              <div>{columns[0]}</div>
              <div>{columns[1]}</div>
            </div>
